...
import { Sparkles, ArrowRight, Play, Compass, Shuffle, MapPin, Music, Receipt, Camera, MessageSquare, AlertTriangle, Calendar } from 'lucide-react';
...
export const OverviewView: React.FC<OverviewViewProps> = ({
  ...
  return (
    <div className="space-y-12 pb-16">
      {/* Overview Top Header & Demo Actions */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[#E2DDD3]">
        <div className="space-y-2">
          ...
          <p className="text-sm text-[#77736C] font-sans max-w-xl">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-[#171717] text-[#F7F4EE] hover:bg-[#333] text-xs font-mono font-bold flex items-center space-x-2 shadow-sm transition-all"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-[#EFEAE0] text-[#171717] hover:bg-[#E2DDD3] border border-[#E2DDD3] text-xs font-mono font-medium flex items-center space-x-2 transition-all"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          { label: 'digital traces', value: metrics.totalTraces, color: 'text-[#171717]' },
          { label: 'places', value: metrics.placesCount, color: 'text-[#2563EB]' },
          { label: 'songs', value: metrics.songsCount, color: 'text-[#7C3AED]' },
          { label: 'purchases', value: metrics.purchasesCount, color: 'text-[#D97706]' },
          { label: 'events', value: metrics.eventsCount, color: 'text-[#EA580C]' },
          { label: 'chapters', value: metrics.chaptersCount, color: 'text-[#059669]' }
        ].map((stat, idx) => (
          <div
            key={idx}
            className="bg-[#EFEAE0]/50 border border-[#E2DDD3] rounded-xl p-4 text-center space-y-1 hover:border-[#171717]/30 transition-colors"
          >
      ...
      {/* HERO MOMENT SECTION */}
      <div className="bg-[#171717] text-[#F7F4EE] rounded-2xl p-6 sm:p-10 shadow-xl space-y-6 relative overflow-hidden">
        <div className="flex items-center justify-between text-xs font-mono text-[#EFEAE0]/70">
          ...
          <span>JUNE 14 &bull; BANDRA WEST</span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 pt-2">
          {[
            ...
            <div key={i} className="bg-[#F7F4EE]/10 p-3 rounded-lg border border-[#F7F4EE]/15 text-xs font-mono space-y-1">
              ...
              <div className="text-[11px] text-[#EFEAE0]/70">{item.detail}</div>
            </div>
          ))}
        </div>

        <div className="pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-[#F7F4EE]/15">
          <p className="text-xs text-[#EFEAE0]/80 font-sans max-w-xl">
            “These records were never designed to tell a story. But together, they do.”
          </p>
          <div className="flex items-center space-x-2 w-full sm:w-auto">
            <button
              onClick={() => onReconstructDay('2026-06-14')}
              className="px-4 py-3 rounded-xl bg-[#F7F4EE]/10 text-[#F7F4EE] hover:bg-[#F7F4EE]/20 font-mono text-xs font-bold border border-[#F7F4EE]/20 transition-all"
            >
              Reconstruct this day &rarr;
            </button>
            <button
              onClick={() => onFollowThread(heroReceipt)}
              className="px-6 py-3 rounded-xl bg-[#F7F4EE] text-[#171717] hover:bg-white font-mono text-xs font-bold flex items-center justify-center space-x-2 transition-all shadow-md group"
            >
      ...
      {/* ANOMALY CARD SNAPSHOT (Priority 3 & 4 Trigger from Overview) */}
        ...
        <div className="p-6 rounded-2xl border border-[#E2DDD3] bg-[#EFEAE0]/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-start space-x-3">
            ...
            <div className="space-y-0.5">
              <span className="text-[10px] font-mono font-bold text-[#E11D48] uppercase">UNUSUAL ACTIVITY DAY DETECTED</span>
